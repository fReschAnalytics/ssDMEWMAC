                ...
                for x in ${DELTAS}; do
                    for w in ${WAITTIMES}; do
                        echo "Running the simulation for Dimension: ${d}, Method ${m}, Lambda: ${l}, Target ARL: ${a}, Delta: ${x}, and a Pertubation at Observation: ${w}." 
                            ...
                            docker run -it -v /Users/robertresch/PycharmProjects/ssDMEWMAC/data:/out -e SIMULATION_TYPE="OOC" -e METH="${m}" -e DIME=${d} -e ITERATIONS=${ITERATIONS} -e LAMBDA=${l} -e ARL=${a} -e DELTA=${x} -e WAIT_TIME=${w} -e LOCAL_OR_GCP="LOCAL" ssdmewmac
                    done
                done
            done
        done
