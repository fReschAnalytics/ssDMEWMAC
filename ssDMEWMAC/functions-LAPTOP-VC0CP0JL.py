...
def find_out_of_control_run_length(method, iterations, dimension, smoothing, h_val, delta, change_after):
    print("""Finding the out of control run length for a %s of dimension %d with smoothing parameter %f with a 
          magnitude %f change at time t = %d.""" % (method, int(dimension), float(smoothing), float(delta), int(change_after) ))

    arl = simulate(method=method, dimension=int(dimension), smoothing=float(smoothing), h=h_val, iterations=int(iterations),
